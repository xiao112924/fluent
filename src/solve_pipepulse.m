...
% ---------- 静压预应力 ----------
% 平均静压在这里独立用于静力预应力几何刚度。
if isfield(cfg,'prestress') && isfield(cfg.prestress,'enabled') && cfg.prestress.enabled
    fprintf('PipePulse：正在计算静压预应力几何刚度...\n');

    p0node = build_static_pressure_profile( ...
        mesh,inletNode,outletNode,Hin.mean,Hout.mean);

    PS = solve_static_prestress_state( ...
        mesh,cfg,S,p0node,inletNode,outletNode);

    S.KG = PS.KG;
    S.Kt = S.K + S.KG;

    % 保持原Rayleigh alpha/beta，刚度比例阻尼项采用切线刚度。
...
bendTable = table((1:numel(bends)).',...
    [bends.R].',[bends.theta].'*180/pi,[bends.fit_error].',...
